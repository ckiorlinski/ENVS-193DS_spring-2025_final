...
model1_predictions <- ggpredict(
  model1,
  terms = c("edge.distance [all]", "season")  # x-axis: edge.distance, grouping: season
)

df_preds <- as.data.frame(model1_predictions)
...
ggplot(data = df_preds,
  ...
  labs(x = "Distance from forest edge (m)",
       ...
       title = "Forest Edge Effects on Swift Parrot Nest Occupancy",
       caption = my_caption
  ) +
  theme_bw() +
  theme(
    ...
    plot.caption     = element_text(hjust = 0.5, size = 9, margin = margin(t = 5, b = 0)),
...
df_preds_0900 <- as.data.frame(model1_pred_0900)
